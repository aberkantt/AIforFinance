...
def main():
    ...
    st.sidebar.write("Berkant Öntemel")

    # İletişim bilgilerini sidebar içinde görüntüledim
    st.sidebar.subheader("E-posta:")
    st.sidebar.write("berkant.ontemel@ogr.sakarya.edu.tr")
    st.sidebar.subheader("Linkedln:")
    st.sidebar.write("https://www.linkedin.com/in/berkant-ontemel/")
    st.sidebar.subheader("Github:")
    st.sidebar.write("https://github.com/aberkantt")
...
plt.plot(close_prices, label='Kapanış Fİyatı')
plt.legend()
...
# Kapanış fiyatlarının 5 günlük hareketli standart sapmalarını hesapladım
...
for i in range(len(standard_deviations)):
    # Üst sınırı, kapanış ortalaması artı iki kat standart sapma olarak ehsapladım
    upper_bound = close_avg[i] + (standard_deviations[i] * 2)

    # Alt sınırı, kapanış ortalamasından iki kat standart sapma çıkartarak hesapaldım
...
for i in range(len(close_for_calc)):
    if i == 0:
        pivot_points.append(float("nan")) # ilk pivot noktasını NaN olarak ayarladım
    else:
        prev_high = high_prices[i - 1]
        prev_low = low_prices[i - 1]
        prev_close = close_prices[i - 1]

        # Pivot noktasını önceki yüksek, düşük ve kapanış fiyat ortalaması olarak hesapladım
...
for i in range(len(pivot_points)):
    if i == 0:
        ...
        support_1.append(float("nan")) # İlk destesk seviyesini NaN olarak ayarladım
    else:
        prev_high = high_prices[i - 1]
        prev_low = low_prices[i - 1]
...
plt.title("Destek ve Direnç",fontsize = 15,color='black')
plt.xlabel("Gün Sonrası", fontsize = 15,color='black')
plt.ylabel("Fiyat", fontsize=15,color="black")
plt.legend()
fig_html = mpld3.fig_to_html(fig)
...
# Kullanıcıdan gelecek gün sayısı girin (20 yi geçmesin)
FUTURE_DAYS = st.text_input("Gelecek gün sayısını girin (20 günü geçmesin)")
...
if export_as_pdf:
    ...
    pdf.multi_cell(w=0, h=7,
                   txt=f"Bu rapor, {ticker} hissesini cesitli teknik gostergeleri ve diger teknikleri kullanarak analiz edecek ve verilen hissenin gelecekteki egilimleri hakkinda fikir verecektir.")
    pdf.ln(50)
    pdf.ln(50)
    ...
    pdf.multi_cell(
        w=0, h=7,
        txt=f"Bollinger bantlari kullanarak hisse senedi piyasasinin volatilitesi hakkinda bir fikir edinmek mumkun ve herhangi bir buyuk trendin olup olmadigini takip etmek mumkun. RSI ile birlestirildiginde, bir hissenin durumu hakkinda cok net bir resim elde edebiliriz.")
    pdf.ln(10)
    with NamedTemporaryFile(delete=False, suffix=".png") as tmpfile:
